...
//    private onDuerChangeListener onDuerChangeListenen;
    ...
    public SceneryCamGLRender(Context context, SurfaceTexture.OnFrameAvailableListener listener) {
...
//                MToast.show(mContext, "暂时无法获取您的位置");
            ...
            if (mArChangeListener != null) {
                mArChangeListener.azimuthChanged(remapValue[0]);
                if (mOldY == 0 || mOldX == 0 || mX != mOldX || mY != mOldY) {
                    mOldX = mX;
                    mOldY = mY;
                    mArChangeListener.locChanged(mX, mY);
                }
            }
            if (arPoiList.size() < 2) {
                if (mArChangeListener != null) {
            ...
            Collections.sort(selectPois, new Comparator<ArPoiScenery>() {
                @Override
    ...
    private void moveItem(final int i, float[] remapValue, final RelativeLayout rl_camview,
        ...
        final SceneryGLPOITexture itemGLPoi = (SceneryGLPOITexture) mPoiList.get(i);
        itemGLPoi.setLoc((long) selectPois.get(i).getPoint().getPoint_x(),
                (long) selectPois.get(i).getPoint().getPoint_y(), 100);
        // 给GL模型设值
        itemGLPoi.setAzimuth(mX, mY, selectPois.get(i).getName());
...
//            }
    ...
    private SceneryGLPOITexture newPOI() throws GLException {
        SceneryGLPOITexture mGLPOITexture = new SceneryGLPOITexture(mSurfaceWidth, mSurfaceHeight);
...
//        }
    ...
    private ArrayList<ArPoiScenery> selectPoiList( ArrayList<ArPoiScenery> arPoiList) throws Exception {
        ...
        ArrayList<ArPoiScenery> selectPoiArray = new ArrayList<>();
        int sum = 0;
        // 排序
        Collections.sort(arPoiList, new Comparator<ArPoiScenery>() {
            @Override
            public int compare(ArPoiScenery lhs, ArPoiScenery rhs) {
                try {
                    // 升序
